...
struct PageState {
   private:
    mutex mtx_locked;
    mutex mtx_s;
    condition_variable cv_locked;
    condition_variable cv_s;

   public:
    struct proxy : atomic<u64> {
        friend class PageState;

       private:
        using atomic<u64>::compare_exchange_strong;
        using atomic<u64>::compare_exchange_weak;
        using atomic<u64>::store;
    };
    proxy stateAndVersion;
    ...
    void unlockX() {
        ...
        // must acquire and release lock before notify, in case of race condition in wait
        { lock_guard<mutex> lk(mtx_locked); }
    ...
    bool tryLockS(u64 oldStateAndVersion) {
        ...
        if (s == Marked)
            return stateAndVersion.compare_exchange_strong(oldStateAndVersion,
                                                           sameVersion(oldStateAndVersion, 1));
        return false;
    }
    ...
    bool tryUpgrade(u64 v) {
        return (getState(v) == 1) &&
               stateAndVersion.compare_exchange_weak(v, sameVersion(v, Locked));
    }

    bool unMark(u64 v) {
        assert(getState(v) == Marked);
        return stateAndVersion.compare_exchange_weak(v, sameVersion(v, Unlocked));
    }
    ...
    void wait() {
        unique_lock<mutex> lk(mtx_locked);
        cv_locked.wait(lk, [&] { return getState() != Locked; });
    }

    void wait_s() {
        unique_lock<mutex> lk(mtx_s);
        cv_s.wait(lk, [&] {
            u64 s = getState();
            return !(s >= 1 && s <= MaxShared);
        });
    }
...
// open addressing hash table used for second chance replacement to keep track of
...
struct GuardO {
    ...
    // constructor
    explicit GuardO(u64 pid) : pid(pid), ptr(reinterpret_cast<T*>(bm.toPtr(pid))) { init(); }
        ...
        for (u64 repeatCounter = 0;; repeatCounter++) {
            ...
            switch (PageState::getState(v)) {
                case PageState::Marked: {
                    if (ps.unMark(v)) {
                        version = PageState::sameVersion(v, PageState::Unlocked);
                        return;
                    }
                    break;
                }
    ...
    void checkVersionAndRestart() {
        if (pid != moved) {
            ...
            if ((stateAndVersion << 8) == (version << 8)) {  // same version
                ...
                if (state <= PageState::MaxShared) return;  // ignore shared locks
                if (state == PageState::Marked) {
                    ...
                    if (ps.stateAndVersion.compare_exchange_weak(
                        ...
                        return;  // mark cleared
                    */
                    if (ps.unMark(stateAndVersion)) return;
                }
            }
        ...
        for (u64 repeatCounter = 0;; repeatCounter++) {
            ...
            switch (state) {
                ...
                case PageState::Locked:
                    ...
                    continue;
                default:
                    assert(state != PageState::Evicted);
                    ps.wait_s();
                    continue;
            }
            yield(repeatCounter);
...
BufferManager::BufferManager()
    ...
    u64 virtAllocSize =
        virtSize +
        (1 << 16);  // we allocate 64KB extra to prevent segfaults during optimistic reads
    ...
    if (useExmap) {
        ...
        virtMem =
            (Page*)mmap(NULL, virtAllocSize, PROT_READ | PROT_WRITE, MAP_SHARED, exmapfd, 0);
    } else {
    ...
    cerr << "vmcache "
         << "blk:" << path << " virtgb:" << virtSize / gb << " physgb:" << physSize / gb
         << " exmap:" << useExmap << endl;
}
...
Page* BufferManager::fixX(PID pid) {
    ...
    for (u64 repeatCounter = 0;; repeatCounter++) {
        ...
        switch (PageState::getState(stateAndVersion)) {
            ...
            case PageState::Marked:
            case PageState::Unlocked:
                if (ps.tryLockX(stateAndVersion)) return virtMem + pid;
                break;
            case PageState::Locked:
                ps.wait();
                continue;
    ...
    for (u64 repeatCounter = 0;; repeatCounter++) {
        ...
        switch (PageState::getState(stateAndVersion)) {
            ...
            case PageState::Evicted:
                ...
                break;
            default:
                if (ps.tryLockS(stateAndVersion)) return virtMem + pid;
        }
        yield(repeatCounter);
    }
}
...
void BufferManager::evict() {
    ...
    toEvict.erase(std::remove_if(toEvict.begin(), toEvict.end(),
                                 [&](PID pid) {
                                     PageState& ps = getPageState(pid);
                                     u64 v = ps.stateAndVersion;
                                     return (PageState::getState(v) != PageState::Marked) ||
                                            !ps.tryLockX(v);
                                 }),
                  toEvict.end());
...
struct BTreeNode : public BTreeNodeHeader {
    ...
    void updateHint(unsigned slotId) {
        unsigned dist = count / (hintCount + 1);
        unsigned begin = 0;
        if ((count > hintCount * 2 + 1) && (((count - 1) / (hintCount + 1)) == dist) &&
            ((slotId / dist) > 1))
            begin = (slotId / dist) - 1;
    ...
    bool mergeNodes(unsigned slotId, BTreeNode* parent, BTreeNode* right) {
        ...
        unsigned spaceUpperBound =
            ...
            (reinterpret_cast<u8*>(slot + count + right->count) - ptr()) + leftGrow +
            rightGrow;
    ...
    void splitNode(BTreeNode* parent, unsigned sepSlot, span<u8> sep) {
        ...
        if (isLeaf) {
            ...
            // in inner node split, separator moves to parent (count == 1 + nodeLeft->count +
            // nodeRight->count)
    ...
    SeparatorInfo findSeparator(bool splitOrdered) {
        ...
        if (isInner()) {
            ...
            return SeparatorInfo{static_cast<unsigned>(prefixLen + slot[slotId].keyLen),
                                 slotId, false};
        }
        ...
        return SeparatorInfo{static_cast<unsigned>(prefixLen + slot[bestSlot].keyLen),
                             bestSlot, false};
    }
...
struct BTree {
   private:
    void trySplit(GuardX<BTreeNode>&& node, GuardX<BTreeNode>&& parent, span<u8> key,
                  unsigned payloadLen);
...
void BTree::trySplit(GuardX<BTreeNode>&& node, GuardX<BTreeNode>&& parent, span<u8> key,
                     unsigned payloadLen) {
    ...
    if (parent->hasSpaceFor(
            sepInfo.len,
            sizeof(PID))) {  // is there enough space in the parent for the separator?
    ...
    for (u64 repeatCounter = 0;; repeatCounter++) {
        try {
            ...
            GuardO<BTreeNode> node(
                reinterpret_cast<MetaDataPage*>(parent.ptr)->getRoot(slotId), parent);
...
struct vmcacheAdapter {
   ...
   public:
    void scan(
        ...
        tree.scanAsc({k, l}, [&](BTreeNode& node, unsigned slot) {
            ...
            return found_record_cb(
                typedKey, *reinterpret_cast<const Record*>(node.getPayload(slot).data()));
        });
    }
    // -------------------------------------------------------------------------------------
    void scanDesc(
    ...
    void update1(const typename Record::Key& key, Fn fn) {
        ...
        u16 l = Record::foldKey(k, key);
        tree.updateInPlace(
            {k, l}, [&](span<u8> payload) { fn(*reinterpret_cast<Record*>(payload.data())); });
    }
    // -------------------------------------------------------------------------------------
...
int main(int argc, char** argv) {
    ...
    auto statFn = [&]() {
        ...
        for (uint64_t i = 0; i < runForSec; i++) {
            ...
            cout << cnt++ << "," << prog << "," << rmb << "," << wmb << "," << systemName
                 << "," << nthreads << "," << n << "," << (isRndread ? "rndread" : "tpcc")
                 << "," << bm.batch << endl;
        }
        keepRunning = false;
    };
    ...
    TPCCWorkload<vmcacheAdapter> tpcc(warehouse, district, customer, customerwdl, history,
                                      neworder, order, order_wdc, orderline, item, stock, true,
                                      warehouseCount, true);
